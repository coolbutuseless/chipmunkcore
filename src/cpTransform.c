...
SEXP cpTransformMult_(SEXP t1_, SEXP t2_)  {
  cpTransform *t1 = TYPEOF(t1_) != EXTPTRSXP ? NULL : (cpTransform *)R_ExternalPtrAddr(t1_);
  if (t1 == NULL) error("'cpTransform * t1' pointer is invalid/NULL");
  cpTransform *t2 = TYPEOF(t2_) != EXTPTRSXP ? NULL : (cpTransform *)R_ExternalPtrAddr(t2_);
...
SEXP cpTransformTranslate_(SEXP translate_)  {
  cpVect *translate = TYPEOF(translate_) != EXTPTRSXP ? NULL : (cpVect *)R_ExternalPtrAddr(translate_);
